...
def apply_welch(raw, fs):
    data = raw.get_data()
    nperseg = int(2 * fs)
    freqs, power_spectrum = welch(data, fs, nperseg=nperseg, axis=1)
    return freqs, power_spectrum

output_dir = 'Wykresy_welch'
...
for band_name, (low_freq, high_freq) in frequency_bands.items():
    ...
    freqs_rest, power_rest = apply_welch(raw_rest_copy, fs)
    ...
    freqs_bridge, power_bridge = apply_welch(raw_bridge_copy, fs)
    ...
    freqs_chess, power_chess = apply_welch(raw_chess_copy, fs)
